...
6. once installed we are going to configure the linux server to have " ip forwarding " ( server will act as an exit node) . This wil give the clients to connect to the vpn with the server beeing as an exit node , meaning all traffic will go through the server ( default gateway of the server ) and not to the wifi that you are using 
...
7. if your Linux system has a /etc/sysctl.d directory, use:
 ...
 8. IF you are using ufw firewall :
   ...
   . sudo ufw allow from 100.64.0.0/10 to any port <your services port proto <protocol> or the tailscal ip you want to allow ( make sure the client and the server has static tailscale ip )
...
In your nginx config you should allow the 100.64.0.0/10 ip range for every service for the vpn or the client tailscale statis ip
...
IN THE SIDE DISABLE ALSO THE tailscale ssh . we don't need it because we can ssh through port 22
    ...
    !!!!!IMPORTANT!!!! 11. In tailscale go to DNS -> nameservers . remove everything expept the Magic dns . click the " add name server " -> " custome " and insert the tailscale ip of the server . Then enable ONLY the " restrict dns ( split dns ) " and add the domain you want . ( it is the local dns server of tailscale ) . DO NOT ENABLE THE " Use with exit node " .  on your local dns server ( not tailscale ) you should allow ON ALL INTERFACES ( LAN AND TAILSCALE INTERFACE ) THE 53/udp and 53/tcp dns port ( see docker .yaml for pi-hole dns )
